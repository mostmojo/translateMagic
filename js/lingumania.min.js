	...
	function translateDOM() {
		if (linguJSON) {
			...
			if (1 < languages.length) {
				...
				for (
					var currlangcode = '' != translationLang ? translationLang : sourceLang.lang_code, j = 1, i = 0;
					i < languages.length;
					i++
				) {
				...
				if (
					...
					(translateNowItem.href = 'http://www.empiremedia.com'),
					(translateNowItem.innerHTML = 'Powered by Empire Media'),
				...
				for (var links = d.getElementsByTagName('a'), i = 0; i < links.length; i++) {
					...
					if (link.attributes.href && 'notranslate' != link.parentNode.className) {
						...
						if (
							...
							if (linguJSON.translated_pages) {
								for (
									var comparableLink = createComparableLink(absoluteLink, languages), j = 0;
