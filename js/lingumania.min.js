...
!(function (w, d, u) {
	...
	getQSParameterByName('lang', w.location.href) ?
		...
		2 == parseURL(w.location.href, !1).substring(0, 4).replace(/\//g, '').length ?
		(translationLang = parseURL(w.location.href, !1).substring(0, 4).replace(/\//g, '')) :
		2 == parseURL(w.location.href, !0).substring(0, parseURL(w.location.href, !0).indexOf('.')).length ?
		(translationLang = parseURL(w.location.href, !0).substring(0, parseURL(w.location.href, !0).indexOf('.'))) :
		2 ==
	...
	function parseURL(e, t) {
		...
		return (
			...
			((a = e.split('/')[2]), t || (a = e.split('/')[0] + '//' + e.split('/')[2])) :
			(a = e.split('/')[0]),
			t ? a : e.replace(a, '')
		);
	}
		...
		'?' == n
			...
			(-1 != e.attributes.href.value.indexOf('#') &&
				((r = e.attributes.href.value.split('#')[1]),
					(e.attributes.href.value = e.attributes.href.value.replace('#' + r, ''))),
				getQSParameterByName('lang', e.attributes.href.value) ?
				(e.attributes.href.value = e.attributes.href.value
					.toLowerCase()
					.replace('lang=' + getQSParameterByName('lang', e.attributes.href.value), 'lang=' + a)) :
				-1 != e.attributes.href.value.indexOf('?') ?
			...
			(e.attributes.href.value = t.split('/')[0] + '//' + parseURL(t, !0) + parseURL(t, !1)) :
			(e.attributes.href.value = t.split('/')[0] + '//' + parseURL(t, !0) + '/' + a + parseURL(t, !1)) :
	...
	function encodeAllSpecialTags(e) {
		...
		if (a)
			for (var n = 0; n < a.length; n++)
				e = e.replace(t, '&lt;' + a[n].substring(1, a[n].length - 1).toLowerCase() + '&gt;');
	...
	function translateDOM() {
		if (linguJSON) {
			...
			if (1 < languages.length) {
				...
				for (
					var currlangcode = '' != translationLang ? translationLang : sourceLang.lang_code, j = 1, i = 0; i < languages.length; i++
				) {
					if (languages[i].lang_code != currlangcode) {
						var href =
							w.location.protocol + '//' + languages[i].url_pattern + parseURL(w.location.href, !1),
							fragment;
						if (languages[i].url_pattern && '?' != languages[i].url_pattern)
							'/' == languages[i].url_pattern &&
							(href =
								2 ==
								parseURL(w.location.href, !1).substring(0, 4).replace(/\//g, '').toLowerCase()
								.length ?
								w.location.protocol +
								'//' +
				...
				if (
					((translateNowItem.className = 'lingumania_poweredby'),
						...
						(translateNowItem.href = 'http://mojodigital.io'),
						(translateNowItem.innerHTML = 'Powered by Mojo Digital'),
						langMenu.appendChild(translateNowItem),
						(menuHeight += 30),
						linguJSON.custom_lang_switcher_html && linguJSON.custom_lang_switcher_container_id)
				) {
					translateNowItem.id = 'lingumania_custom_id';
					var frag = d.createDocumentFragment();
					for (customLangMenu.innerHTML = customLangMenuHtml; customLangMenu.firstChild;)
			...
			if (
					...
					if (null != node.nodeValue) {
						...
						for (var canBeTranslated = !0, current = node; canBeTranslated && current.parentNode;)
							...
							for (var i = 0; i < current.attributes.length; i++)
								'notranslate' == current.attributes[i].value && (canBeTranslated = !1);
						if (canBeTranslated)
							try {
								for (
									var startingWhiteSpaceRegex = /^\s+/gim,
										startingWhiteSpaceMatches = startingWhiteSpaceRegex.exec(node.nodeValue),
										endingWhiteSpaceRegex = /\s+$/gim,
										endingWhiteSpaceMatches = endingWhiteSpaceRegex.exec(node.nodeValue),
										i = 0; i < translatedSegments.length; i++
								)
									if (
										...
										if (
											(startingWhiteSpaceMatches &&
												...
												endingWhiteSpaceMatches && (target += endingWhiteSpaceMatches[0]),
												target.match(
													/<\/?\w+((\s+\w+(\s*=\s*(?:".*?"|'.*?'|[\^'">\s]+))?)+\s*|\s*)\/?>/gim
												))
										) {
											...
											for (wrap.innerHTML = target.replace(/\\"/g, '"'); wrap.firstChild;)
				...
				for (var links = d.getElementsByTagName('a'), i = 0; i < links.length; i++) {
					...
					if (link.attributes.href && 'notranslate' != link.parentNode.className) {
						...
						if (
							-1 != link.attributes.href.value.indexOf(parseURL(w.location.href, !0)) ||
							-1 != link.attributes.href.value.indexOf(sourceLang.url_pattern) ||
							...
							if (linguJSON.translated_pages) {
								for (
									var comparableLink = createComparableLink(absoluteLink, languages), j = 0; j < linguJSON.translated_pages.length; j++
								)
				...
				for (var i = 0; i < nodes.length; i++)
					nodes[i].parentNode && nodes[i].parentNode.replaceChild(fragments[i], nodes[i]);
			}
			if (
						...
						for (var j = 0; j < translatedImageSegments.length; j++)
							if (
								(null == translatedImageSegments[j].img_target &&
									...
									.replace('https://', '')
									.endsWith(
										img.attributes.src.value.trim().replace('http://', '').replace('https://', '')
									))
							) {
	...
	function createComparableLink(e, t) {
		...
		return (
			(a != parseURL(w.location.href, !0) && a != t[0].url_pattern) || (a = '/'),
	...
	(TreeWalker.prototype.firstChild = function () {
		for (
			var e = this.currentNode.firstChild; e && this._getFilteredStatus(e) !== this.NodeFilter.FILTER_ACCEPT;
	...
	(TreeWalker.prototype.lastChild = function () {
		for (
			var e = this.currentNode.lastChild; e && this._getFilteredStatus(e) !== this.NodeFilter.FILTER_ACCEPT;
	...
	(TreeWalker.prototype.nextSibling = function () {
		for (
			var e = this.currentNode; e &&
			(e.nextSibling && (e = e.nextSibling), this._getFilteredStatus(e) !== this.NodeFilter.FILTER_ACCEPT);
	...
	var linguLoader = function () {
		var e =
			"a.lingumania_target, a.lingumania_poweredby { display: none; background-color: #999; } #lingumania_custom_id { display: block; color: #fff; display: block; width: 160px; padding: 5px; text-decoration: none; } a.lingumania_target:hover, a.lingumania_poweredby:hover { background-color: rgba(0, 0, 0, 0.25); } #lingumania_langswitcher{ position: absolute; top: 0px; right: 0px; z-index: 100001; text-transform: uppercase; text-align: left; color: #fff; font-size: 24px; line-height: 18px; } #lingumania_langswitcher:hover a { display: block; position: relative; z-index: 100002; float: right; width: 160px; padding: 5px; clear: both; color: #fff; text-decoration: none; } a#lingumania_currentlanglink { display: block; width: 160px; padding: 5px; background: #999 url('//az596610.vo.msecnd.net/arrow-down-black.png') right top no-repeat; color: #fff; font-weight: bold; text-decoration: none; }  .lingumania_poweredby { background: url('//https://imgur.com/a/iFnbji9') right center no-repeat; border-top: 1px solid #808080; font-size: 9px; line-height: 12px; }",
